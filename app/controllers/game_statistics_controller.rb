class GameStatisticsController < ApplicationController
  ...
  def close_game
	...
	if @active_game.agreement_status == true
	  redirect_to end_negotiation_path(:active_game=>@active_game.id)
	else
  ...
  def end_negotiation_update
	...
	@explayer_scorecards = PlayerScorecard.all(:conditions=>['player_id=?', @player.id])
	@explayer_scorecards.each do |scorecard|
	  scorecard.destroy
	end
	...
	if @player_scorecards.all?(&:valid?)
	  ...
	  @all_issues.each do |issue|
		@base_scorecard=PlayerScorecard.find(:first, :conditions=>['issue_id=?', issue.id])
	  ...
	  @active_game.save
	  if @active_game.status
		redirect_to :action=>'agreement_mismatch', :active_game=>@active_game.id
	  else
		redirect_to :action=>'game_over', :active_game=>@active_game.id
	  end
	else
	  redirect_to root_path(:active_game=>@active_game.id)
	end
  end
  ...
  def download_email
	...
	i=0
	@messages=Message.find(:all, :conditions => ["author_id IN (?)", @players])
	...
	@to=Array.new
	@messages.each do |m|
	  @names << Player.find(:first, :conditions=>['user_id=? and game_id=?', m.author_id, @game.id]).role.name
	end
  ...
  def general_statistics
	...
	@agreement_values.each do |value|
	  if (@min_agreement_value > value)
		@min_agreement_value=value

	  end

	  if (@max_agreement_value < value)
